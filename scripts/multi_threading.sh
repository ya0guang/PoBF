...
if [[ $2 = "gramine" || $2 = "all" ]]; then
    for i in $(seq 1 $TIMES); do
        for task in "${tasks[@]}"; do
            ...
            > ../../../data/$task/"$i"mt_output_data_provider_gramine.txt 2>&1
            unset DATA_PATH
...
if [[ $2 = "pobf" || $2 = "all" ]]; then
    for i in $(seq 1 $TIMES); do
        for task in "${tasks[@]}"; do
            ...
            { time ./app $ADDRESS $PORT; } > ../../../data/$task/"$i"mt_output_enclave_pobf.txt 2>&1 &
            sleep 1
            popd > /dev/null
            
            pushd ./data_provider/bin > /dev/null
            { time eval 'parallel -j0 -N0 ./data_provider run ../../data/$task/manifest.json ::: '{1..$1}''; } \
            > ../../data/$task/"$i"mt_output_data_provider_pobf.txt 2>&1
            popd > /dev/null
...
if [[ $2 = "native" || $2 = "all" ]]; then
    for i in $(seq 1 $TIMES); do
        for task in "${tasks[@]}"; do
            ...
            pushd ./data_provider/bin > /dev/null
            { time eval 'parallel -j0 -N0 ./data_provider run ../../data/$task/manifest.json ::: '{1..$1}''; } \
            > ../../data/$task/"$i"mt_output_data_provider_native.txt 2>&1
            popd > /dev/null
