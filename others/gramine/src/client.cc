 ...
 * This program is originally based on an mbedTLS example ssl_client1.c but uses
...
typedef int (*ra_tls_ptr_der)(uint8_t*, size_t);
typedef void (*ra_tls_ptr_measurement)(int (*f_cb)(const char*, const char*,
...
static std::string read_file() {
  ...
  if (getenv("DATA_PATH") == nullptr) {
    path = "../../data/task_sample/data.bin";
  } else {
...
static int parse_hex(const char* hex, void* buffer, size_t buffer_size) {
  if (strlen(hex) != buffer_size * 2) return -1;

  for (size_t i = 0; i < buffer_size; i++) {
    if (!isxdigit(hex[i * 2]) || !isxdigit(hex[i * 2 + 1])) return -1;
...
static int my_verify_measurements(const char* mrenclave, const char* mrsigner,
  ...
  if (g_verify_isv_prod_id && memcmp(isv_prod_id, g_expected_isv_prod_id,
                                     sizeof(g_expected_isv_prod_id)))
    return -1;
...
static int my_verify_callback(void* data, mbedtls_x509_crt* crt, int depth,
                              uint32_t* flags) {
  (void)data;

  if (depth != 0) {
    /* the cert chain in RA-TLS consists of single self-signed cert, so we
     * expect depth 0 */
    return MBEDTLS_ERR_X509_INVALID_FORMAT;
  }
  if (flags) {
    /* mbedTLS sets flags to signal that the cert is not to be trusted (e.g., it
     * is not correctly signed by a trusted CA; since RA-TLS uses self-signed
     * certs, we don't care what mbedTLS thinks and ignore internal cert
     * verification logic of mbedTLS */
    *flags = 0;
  }
...
int main(int argc, char** argv) {
  ...
  std::string data_len = std::to_string(data.size());
  int ret;
  size_t len;
  ...
  if (!strcmp(argv[1], "epid")) {
    ...
    if (!ra_tls_verify_lib) {
      mbedtls_printf("%s\n", dlerror());
      mbedtls_printf(
          "User requested RA-TLS verification with EPID but cannot find lib\n");
      if (in_sgx) {
        mbedtls_printf(
            "Please make sure that you are using client_epid.manifest\n");
      }
      return 1;
    }
    ...
    if (in_sgx) {
      /*
       * RA-TLS verification with DCAP inside SGX enclave uses dummies instead
       * of real functions from libsgx_urts.so, thus we don't need to load this
       * helper library.
       */
      ...
      if (!ra_tls_verify_lib) {
        mbedtls_printf("%s\n", dlerror());
        mbedtls_printf(
            "User requested RA-TLS verification with DCAP inside SGX but "
            "cannot find lib\n");
        mbedtls_printf(
            "Please make sure that you are using client_dcap.manifest\n");
        return 1;
      }
    } else {
      void* helper_sgx_urts_lib =
          dlopen("libsgx_urts.so", RTLD_NOW | RTLD_GLOBAL);
  ...
  if (ra_tls_verify_lib) {
    ra_tls_verify_callback_der_f = (ra_tls_ptr_der)(dlsym(
        ra_tls_verify_lib, "ra_tls_verify_callback_der"));
    if ((error = dlerror()) != NULL) {
      mbedtls_printf("%s\n", error);
      return 1;
    }

    ra_tls_set_measurement_callback_f = (ra_tls_ptr_measurement)(dlsym(
  ...
  if (argc > 2 && ra_tls_verify_lib) {
    if (argc != 6) {
      mbedtls_printf(
          ...
          "       (first two in hex, last two as decimal; set to 0 to "
          "ignore)\n",
          argv[0], argv[1]);
      return 1;
    }
    ...
    } else if (parse_hex(argv[2], g_expected_mrenclave,
                         sizeof(g_expected_mrenclave)) < 0) {
    ...
    } else if (parse_hex(argv[3], g_expected_mrsigner,
                         sizeof(g_expected_mrsigner)) < 0) {
  ...
  if (ra_tls_verify_lib) {
    /* use RA-TLS verification callback; this will overwrite CA chain set up
     * above */
  ...
  while ((ret = mbedtls_ssl_write(&ssl, (uint8_t*)(data_len.c_str()),
                                  data_len.size())) <= 0) {
    if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
  ...
  while ((ret = mbedtls_ssl_write(&ssl, (uint8_t*)(data.c_str()),
                                  data.size())) <= 0) {
    if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
      mbedtls_printf(" failed\n  ! mbedtls_ssl_write returned %d\n\n", ret);
      goto exit;
