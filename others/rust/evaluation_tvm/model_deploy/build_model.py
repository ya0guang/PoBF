...
RUNTIMES = [
    (Runtime("crt", {"system-lib": True}), "{name}.{ext}")
]
    ...
    for runtime, file_format_str in RUNTIMES:
        with tvm.transform.PassContext(opt_level=3, config={"tir.disable_vectorize": True}):
            graph, lib, params = relay.build(
                func, "llvm", runtime=runtime, params=params)
        ...
        with open(
            os.path.join(build_dir, file_format_str.format(
                name="graph", ext="json")), "w"
        ) as f_graph_json:
            f_graph_json.write(graph)
        with open(
            os.path.join(build_dir, file_format_str.format(
                name="params", ext="bin")), "wb"
        ) as f_params:
