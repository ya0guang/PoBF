...
std::vector<unsigned char> base64_to_binary(const std::string& base64_data) {
  ...
  return boost::algorithm::trim_right_copy_if(
      std::vector<unsigned char>(It(std::begin(base64_data)),
                                 It(std::end(base64_data))),
      [](char c) { return c == '\0'; });
}
...
std::string binary_to_base64(const std::vector<unsigned char>& binary_data) {
  ...
  auto tmp =
      std::string(It(std::begin(binary_data)), It(std::end(binary_data)));
...
std::vector<unsigned char> base64url_to_binary(const std::string& base64_data) {
  ...
  // use stanard base64 decode. we dont need to add padding characters.
