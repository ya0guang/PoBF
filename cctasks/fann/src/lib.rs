...
pub fn private_computation(input: Vec<u8>) -> Vec<u8> {
    let n = u32::from_ne_bytes(input[..4].try_into().unwrap());
    let perm = Perm::new(n);
    let mut futures = vec![];
    ...
    for j in (0..).map(|x| x * k).take_while(|&j| j < k * n) {
        ...
        futures.push(work(perm, j as usize, max as usize))
    }
    ...
    let mut ans = Vec::new();
    ans.extend_from_slice(&checksum.to_ne_bytes());
    ans.extend_from_slice(&maxflips.to_ne_bytes());
